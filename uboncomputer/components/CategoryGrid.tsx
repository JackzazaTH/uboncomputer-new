        ...
        <section className="bg-white py-6 border-b border-gray-100">
            <div className="container mx-auto px-4 sm:px-6 2xl:px-8">
                <div className="flex items-center mb-4">
                    <div className="p-1.5 bg-orange-100 rounded-lg mr-3 shadow-sm">
                        <LayoutGrid className="w-5 h-5 text-orange-600" />
                    </div>
                    <h2 className="text-xl font-bold text-gray-900 tracking-tight">หมวดหมู่สินค้า</h2>
                </div>
                    ...
                    {categories.map((category) => {
                        return (
                            <button 
                                ...
                                className="flex flex-col items-center justify-center p-2 bg-white rounded-lg border border-gray-100 hover:border-orange-200 hover:shadow-md hover:-translate-y-1 transition-all duration-200 group h-full"
                            >
                                <div className={`p-2 rounded-lg transition-colors mb-1.5 ${category.color} group-hover:scale-105 duration-200`}>
                                    {React.isValidElement(category.icon) ? React.cloneElement(category.icon as React.ReactElement<any>, { size: 20, strokeWidth: 1.5 }) : null}
                                </div>
