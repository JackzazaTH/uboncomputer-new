...
const SearchOverlay: React.FC<SearchOverlayProps> = ({ isOpen, onClose, onNavigate, allProducts }) => {
        ...
        <div className="flex flex-col h-full">
          <div className="flex items-center h-20 border-b">
            <div className="relative w-full">
             ...
             <button onClick={onClose} className="p-2 ml-2 -mr-2 rounded-full hover:bg-gray-100 transition-colors flex-shrink-0">
          ...
          <div className="flex-grow pt-4 overflow-y-auto">
            {searchTerm.length > 1 && filteredProducts.length === 0 && (
                 <div className="text-center py-16 flex flex-col items-center justify-center h-full">
                    ...
                    <h3 className="text-xl font-bold text-gray-800">ไม่พบผลลัพธ์</h3>
                    <p className="mt-1 text-gray-500">ไม่พบสินค้าสำหรับ "{searchTerm}"</p>
                    <p className="mt-1 text-sm text-gray-400">ลองใช้คำค้นหาอื่นหรือตรวจสอบการสะกด</p>
                </div>
            )}
            {filteredProducts.length > 0 && (
                <ul className="divide-y">
                    {filteredProducts.map(product => (
                        <li key={product.id}>
                            <button onClick={() => handleNavigate(product.id)} className="w-full flex items-center gap-4 p-4 text-left hover:bg-gray-50 rounded-lg">
