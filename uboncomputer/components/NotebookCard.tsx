...
const NotebookCard: React.FC<NotebookCardProps> = ({ product }) => {
    ...
    >
      ...
      <div className="relative mb-4">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent flex items-end justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 pb-4">
            <Link href={`/product/${product.id}`} onClick={(e) => e.stopPropagation()} className="p-3 bg-white/90 rounded-full text-gray-800 hover:bg-white hover:text-orange-500 transition-all scale-90 group-hover:scale-100" title="ดูรายละเอียด">
                <Eye size={20} />
            </Link>
            <button 
