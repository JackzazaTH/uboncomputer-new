...
import { ShoppingCart, CheckCircle, ShieldCheck, Truck, Plus, Minus, XCircle } from 'lucide-react';
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div>
            <nav className="text-sm mb-4" aria-label="Breadcrumb">
              <ol className="list-none p-0 inline-flex">
                <li className="flex items-center">
                  <Link href="/" className="text-gray-500 hover:text-orange-600">หน้าแรก</Link>
                ...
                <li className="flex items-center">
                  <Link href={`/${product.category.toLowerCase()}`} className="text-gray-800 font-semibold hover:text-orange-600">{product.category}</Link>
                </li>
              </ol>
            </nav>
            ...
            <button onClick={handleAddToCart} className="w-full sm:w-auto flex items-center justify-center bg-orange-600 text-white font-bold py-3 px-8 rounded-lg hover:bg-orange-700 transition-colors">
            ...
            <div className="mt-8 border-t pt-6 space-y-3 text-sm text-gray-600">
              {product.stock > 0 ? (
                <div className="flex items-center">
                  ...
                  <span>สถานะ: <span className="font-semibold text-green-600">มีสินค้า ({product.stock} ชิ้น)</span></span>
                </div>
              ) : (
                <div className="flex items-center">
                  <XCircle className="w-5 h-5 text-red-500 mr-2" />
                  <span>สถานะ: <span className="font-semibold text-red-600">สินค้าหมด โปรดติดต่อสอบถาม</span></span>
                </div>
              )}
