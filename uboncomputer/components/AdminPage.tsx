    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
                    ...
                    {localContent.openings.jobs.map(j => (
                       <div key={j.id} className="flex items-center justify-between p-2 bg-gray-100 border rounded-md">
                            ...
                            <div className="flex items-center gap-2">
                                <button onClick={() => handleToggleJobStatus(j.id)} title={j.isActive ? 'ปิดรับสมัคร' : 'เปิดรับสมัคร'} className={`p-1.5 rounded ${j.isActive ? 'text-gray-500 hover:bg-gray-200' : 'text-green-600 hover:bg-green-100'}`}>
                                    {j.isActive ? <ToggleRight size={18}/> : <ToggleLeft size={18}/>}
                                </button>
            ...
            <SectionEditor title="Call to Action Section">
                ...
                <FormInput label="อีเมลสำหรับติดต่อ" name="cta-email" value={localContent.cta.email} onChange={e => handleSimpleChange('cta', 'email', e.target.value)} />
            </SectionEditor>
            
            <JobOpeningForm job={editingJob} onSave={handleJobSave} onClose={() => setIsJobModalOpen(false)} />
...
export const AdminPage: React.FC<AdminPageProps> = (props) => {
  ...
  const sections: { id: AdminSection; name: string; icon: React.ReactNode }[] = [
    { id: 'orders', name: 'คำสั่งซื้อ', icon: <Package size={20} /> },
    { id: 'products', name: 'จัดการสินค้า', icon: <ShoppingCart size={20} /> },
    { id: 'computerSets', name: 'จัดการคอมเซ็ต', icon: <Tv size={20} /> },
    { id: 'articles', name: 'จัดการบทความ', icon: <Newspaper size={20} /> },
    { id: 'homepage', name: 'จัดการหน้าแรก', icon: <Home size={20} /> },
    { id: 'hero', name: 'จัดการ Hero Banner', icon: <ImageIcon size={20} /> },
    { id: 'categoryPages', name: 'จัดการหน้าหมวดหมู่', icon: <LayoutGrid size={20} /> },
    { id: 'discounts', name: 'จัดการโค้ดส่วนลด', icon: <Percent size={20} /> },
    { id: 'careers', name: 'จัดการหน้าสมัครงาน', icon: <Briefcase size={20} /> },
    { id: 'seo', name: 'จัดการ SEO', icon: <Globe size={20} /> },
  ];

  const renderSection = () => {
    switch (activeSection) {
      case 'products':
        return <ManagementPanel title="สินค้าทั้งหมด" onAddNew={handleAddNewProduct}><ProductManagement products={props.products} categories={props.categories} onEdit={handleEditProduct} onDelete={props.onDeleteProduct} /></ManagementPanel>;
      case 'articles':
        return <ArticleManagement articles={props.articles} onSave={props.onSaveArticle} onDelete={props.onDeleteArticle} />;
      case 'hero':
        return <HeroSlideManagement slides={props.heroSlides} onSave={props.onSaveHeroSlide} onDelete={props.onDeleteHeroSlide} />;
      case 'computerSets':
      ...
      case 'seo':
        return <SeoManagement seoData={props.seoData} onSave={props.onSaveSeoData} />;
      case 'homepage':
          return <HomepageManagement content={props.homepageContent} onSave={props.onSaveHomepageContent} />;
      case 'categoryPages':
          return <CategoryPageManagement data={props.categoryPageData} onSave={props.onSaveCategoryPageData} />;
      case 'discounts':
          return <DiscountManagement codes={props.discountCodes} onSave={props.onSaveDiscountCode} onDelete={props.onDeleteDiscountCode} />;
      case 'orders':
          return <OrderManagement orders={props.adminOrders} onSave={props.onSaveAdminOrder} onDelete={props.onDeleteAdminOrder} />;
      case 'careers':
          return <CareersManagement content={props.careersPageContent} onSave={props.onSaveCareersPageContent} />;
      default:
        return <div className="p-8"><h2>เลือกเมนูเพื่อเริ่มจัดการ</h2></div>;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex">
      {/* Sidebar */}
      <aside className={`bg-gray-800 text-white w-64 fixed inset-y-0 left-0 z-30 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 ease-in-out`}>
        <div className="flex items-center justify-between h-20 px-6 bg-gray-900">
          <span className="text-xl font-bold">แผงควบคุม</span>
            ...
            {sections.map(section => (
              <li key={section.id}>
                <button
                  ...
                  className={`w-full flex items-center px-6 py-3 text-left transition-colors ${activeSection === section.id ? 'bg-orange-500 text-white' : 'hover:bg-gray-700'}`}
                >
                  {section.icon}<span className="ml-3">{section.name}</span>
                </button>
              </li>
            ))}
            <li>
              <button onClick={props.onLogout} className="w-full flex items-center px-6 py-3 text-left hover:bg-gray-700">
                <LogOut size={20} /><span className="ml-3">ออกจากระบบ</span>
              </button>
            </li>
          </ul>
        </nav>
      ...
      <main className="flex-1 lg:ml-64">
        {/* Mobile Header */}
        <div className="lg:hidden h-20 bg-white shadow-md flex items-center justify-between px-4 sticky top-0 z-20">
            <button onClick={() => setIsSidebarOpen(true)} className="text-gray-600">
                <Menu size={24} />
            </button>
            <h1 className="text-lg font-bold">{sections.find(s => s.id === activeSection)?.name}</h1>
