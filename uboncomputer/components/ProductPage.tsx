...
import { Product, Page, Review, User, AdminOrder } from '../types';
import { ShoppingCart, CheckCircle, ShieldCheck, Truck, Plus, Minus, XCircle, GitCompare, Clock, Info, LogIn, ShoppingBag } from 'lucide-react';
...
interface ProductPageProps {
  ...
  onSaveReview: (productId: number, reviewData: Omit<Review, 'id' | 'productId' | 'date'>) => void;
...
interface ReviewSectionProps {
  product: Product;
  onSaveReview: ProductPageProps['onSaveReview'];
  isUserLoggedIn: boolean;
  currentUser: User | null;
  adminOrders: AdminOrder[];
  onAuthClick: () => void;
...
const ReviewSection: React.FC<ReviewSectionProps> = ({ product, onSaveReview, isUserLoggedIn, currentUser, adminOrders, onAuthClick }) => {
  const [newReview, setNewReview] = useState({ author: '', rating: 0, comment: '' });
  const { addToast } = useToast();

  useEffect(() => {
    if (isUserLoggedIn && currentUser) {
        setNewReview(prev => ({ ...prev, author: currentUser.name }));
    } else {
  ...
  const hasPurchased = useMemo(() => {
    ...
    // Simulate checking if the current user has purchased this product
    return adminOrders.some(order => 
        order.customer.email === currentUser.email &&
        order.items.some(item => item.id === product.id)
    );
  }, [isUserLoggedIn, currentUser, adminOrders, product.id]);

  const handleReviewSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newReview.author.trim() || !newReview.comment.trim()) {
      addToast('กรุณากรอกชื่อและความคิดเห็น', 'error');
      return;
    }
    if (newReview.rating === 0) {
      addToast('กรุณาให้คะแนนสินค้า', 'error');
      return;
    }
    onSaveReview(product.id, newReview);
    setNewReview({ author: currentUser?.name || '', rating: 0, comment: '' });
    addToast('ขอบคุณสำหรับรีวิวของคุณ!', 'success');
  };
  ...
  const ReviewFormArea = () => {
    if (!isUserLoggedIn) {
      return (
        <div className="text-center p-6 border-2 border-dashed rounded-lg">
          <h4 className="font-semibold text-gray-700">คุณต้องการเขียนรีวิวหรือไม่?</h4>
          <p className="text-sm text-gray-500 my-2">กรุณาเข้าสู่ระบบเพื่อแบ่งปันความคิดเห็นของคุณ</p>
          <button 
            onClick={onAuthClick}
            className="inline-flex items-center gap-2 bg-orange-600 text-white font-bold py-2 px-4 rounded-md hover:bg-orange-700 transition-colors"
          >
            <LogIn size={18} />
            เข้าสู่ระบบ
          </button>
    ...
    if (!hasPurchased) {
        return (
            <div className="text-center p-6 border-2 border-dashed rounded-lg bg-gray-50">
                ...
                <h4 className="font-semibold text-gray-700">ขอบคุณสำหรับความสนใจ!</h4>
                <p className="text-sm text-gray-500 my-2">คุณต้องซื้อสินค้านี้ก่อนจึงจะสามารถรีวิวได้</p>
            </div>
        );
    }
    ...
    return (
        <form onSubmit={handleReviewSubmit} className="space-y-4">
            <div>
                <label htmlFor="author" className="block text-sm font-medium text-gray-700">ชื่อของคุณ*</label>
                <input type="text" id="author" value={newReview.author} readOnly disabled className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 bg-gray-100 text-gray-500 cursor-not-allowed" />
            </div>
            <div>
                <label className="block text-sm font-medium text-gray-700">ให้คะแนน*</label>
                <StarRating rating={newReview.rating} onRatingChange={rating => setNewReview({...newReview, rating})} size={24} />
            </div>
            <div>
                <label htmlFor="comment" className="block text-sm font-medium text-gray-700">ความคิดเห็น*</label>
                <textarea id="comment" rows={4} value={newReview.comment} onChange={e => setNewReview({...newReview, comment: e.target.value})} required className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500 bg-white text-gray-900" />
            </div>
            <button type="submit" className="w-full bg-orange-600 text-white font-bold py-2 px-4 rounded-md hover:bg-orange-700 transition-colors">
                ส่งรีวิว
            </button>
        </form>
    );
  ...
  return (
    <div className="mt-16">
      <div className="border-b mb-8">
        <h2 className="text-2xl font-bold text-gray-900 pb-2 border-b-2 border-orange-500 inline-block">
          รีวิวจากลูกค้า ({reviewCount})
        </h2>
      </div>
        ...
        <div className="lg:col-span-2 space-y-8">
          {reviewCount > 0 ? (
            ...
            <div className="text-center py-10 bg-gray-50 rounded-lg">
              ...
              <p className="text-sm text-gray-400 mt-1">เป็นคนแรกที่รีวิวสินค้านี้!</p>
            </div>
          )}
        </div>

        <div className="lg:col-span-1">
          <div className="bg-gray-50 p-6 rounded-lg sticky top-28 border">
...
const ProductPage: React.FC<ProductPageProps> = ({ product, allProducts, onNavigate, onSaveReview, isUserLoggedIn, currentUser, adminOrders, onAuthClick }) => {
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 2xl:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <p className="text-gray-500 mb-2">แบรนด์: <span className="font-semibold text-gray-700">{product.brand}</span></p>

            <div className="flex items-center mb-4">
                <StarRating rating={averageRating} readOnly />
                <span className="ml-2 text-sm text-gray-600">({reviewCount} รีวิว)</span>
            </div>
             ...
             <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4">
               {product.specs && Object.keys(product.specs).length > 0 ? Object.entries(product.specs).map(([key, value]) => (
                 <li key={key} className="flex border-b pb-2">
                   <span className="font-semibold text-gray-800 w-1/3 uppercase">{key}</span>
