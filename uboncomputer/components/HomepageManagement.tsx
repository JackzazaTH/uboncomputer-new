...
import { HomepageContent, PromoBanner, YoutubeVideo, StoreBranch, InfoBarItem, DualCarouselContent, DisplayProduct, Brand, SocialPlatform, ComputerSetPageContent, ComputerSetPageBanner, Page, Branding, LucideIconName } from '../types';
import ImageUpload from './ImageUpload';
import { FormInput, Modal, FormWrapper } from './AdminPage';
import { PlusCircle, Trash2, ChevronDown, ChevronUp, Save, Edit, Wrench, Computer, Monitor, Printer, Laptop, Cpu, CircuitBoard, Cog, MemoryStick, HardDrive, Power, Box, Fan, Gamepad2, Router, FileCode, Server } from 'lucide-react';
...
const iconMap: Record<LucideIconName, React.ReactNode> = {
    Wrench: <Wrench />, Computer: <Computer />, Monitor: <Monitor />, Printer: <Printer />,
    Laptop: <Laptop />, Cpu: <Cpu />, CircuitBoard: <CircuitBoard />, Cog: <Cog />,
    MemoryStick: <MemoryStick />, HardDrive: <HardDrive />, Power: <Power />, Box: <Box />,
    Fan: <Fan />, Gamepad2: <Gamepad2 />, Router: <Router />, FileCode: <FileCode />, Server: <Server />,
};
...
const HomepageManagement: React.FC<HomepageManagementProps> = ({ content, onSave, computerSetPageContent, onSaveComputerSetPageContent }) => {
  ...
  useEffect(() => {
    setLocalComputerSetContent(computerSetPageContent);
  }, [computerSetPageContent]);
  ...
  const handleBrandingChange = (field: keyof Branding, value: any) => {
      setLocalContent(prev => ({
          ...prev,
          branding: {
              ...prev.branding,
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
      ...
      <SectionEditor title="การตกแต่งร้าน (Branding)">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <FormInput label="ชื่อหลัก (เช่น UBON)" name="brand-first" value={localContent.branding.logoTextFirst} onChange={e => handleBrandingChange('logoTextFirst', e.target.value)} />
              <FormInput label="ชื่อรอง (เช่น COMPUTER)" name="brand-second" value={localContent.branding.logoTextSecond} onChange={e => handleBrandingChange('logoTextSecond', e.target.value)} />
              <FormInput label="คำโปรย (Subtitle)" name="brand-sub" value={localContent.branding.subtitle} onChange={e => handleBrandingChange('subtitle', e.target.value)} />
              <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">ไอคอนโลโก้</label>
                   <select
                      value={localContent.branding.icon}
                      onChange={e => handleBrandingChange('icon', e.target.value)}
                      className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500 bg-white"
                  >
                  ...
                  <div className="mt-2 flex items-center gap-2">
                      ...
                      <div className="p-2 bg-orange-500 text-white rounded-md inline-block">
                        {/* @ts-ignore */}
                         {iconMap[localContent.branding.icon]}
                      </div>
                  </div>
              </div>
          </div>
