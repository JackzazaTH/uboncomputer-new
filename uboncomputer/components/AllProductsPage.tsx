...
import { SlidersHorizontal, List, ShoppingBag } from 'lucide-react';
...
const AllProductsPage: React.FC<AllProductsPageProps> = ({ allProducts, onNavigate }) => {
  ...
  const availableFilters = useMemo(() => {
    ...
    filtersConfig.forEach(filter => {
        const options = [...new Set(allProducts.map(p => getProperty(p, filter.key)).filter(Boolean))].sort();
        if (options.length > 1) {
  ...
  const filteredAndSortedProducts = useMemo(() => {
    ...
    return result;
  }, [allProducts, activeFilters, priceRange, sortOption]);
        ...
        <div className="space-y-6">
          ...
          <div>
             ...
             <div className="flex items-center space-x-2">
                 <input type="number" value={priceRange[0]} onChange={e => setPriceRange([+e.target.value, priceRange[1]])} className="w-full border-gray-300 rounded-md shadow-sm text-sm" placeholder="ต่ำสุด"/>
                 <span>-</span>
                 <input type="number" value={priceRange[1]} onChange={e => setPriceRange([priceRange[0], +e.target.value])} className="w-full border-gray-300 rounded-md shadow-sm text-sm" placeholder="สูงสุด"/>
             </div>
          </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <nav className="text-sm mb-4" aria-label="Breadcrumb">
          <ol className="list-none p-0 inline-flex">
            ...
            <li className="flex items-center">
              <span className="text-gray-800 font-semibold">สินค้าทั้งหมด</span>
            </li>
          </ol>
        </nav>

        <div className="mb-8 relative rounded-lg overflow-hidden text-white flex items-center justify-center h-56 bg-gradient-to-r from-gray-700 via-gray-900 to-black">
             <div className="absolute inset-0 bg-black/30"></div>
             <div className="relative text-center z-10 p-4">
                <ShoppingBag size={48} className="mx-auto mb-2" />
                <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight">สินค้าทั้งหมด</h1>
                <p className="mt-2 text-lg text-white/90">เลือกซื้อสินค้าไอทีทุกชิ้นในร้านของเรา</p>
             </div>
        </div>
          ...
          <main className="2xl:col-span-3">
            <div className="flex flex-col sm:flex-row justify-between items-center mb-6 p-4 bg-white rounded-lg shadow-sm border border-gray-200">
                ...
                <div className="flex items-center space-x-2">
                    ...
                    <select id="sort" value={sortOption} onChange={e => setSortOption(e.target.value)} className="rounded-md border-gray-300 shadow-sm focus:border-orange-500 focus:ring-orange-500 text-sm">
