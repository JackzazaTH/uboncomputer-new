...
import { Product, Category, Article, ComputerSetGroup, HeroSlide, ComputerSetProduct, SeoData, SeoEditablePage, HomepageContent, AllCategoryPageData, ProductCategory, CategoryPageData, Page, DiscountCode, AdminOrder, AdminOrderStatus, CareersPageContent, CareersFeatureCard, JobOpening, ComputerSetSpecs, HeaderMenuData, LucideIconName, CategoryGroup, CategoryLink } from '../types';
import { LogOut, ShoppingCart, Tv, Newspaper, Image as ImageIcon, PlusCircle, Edit, Trash2, X, ChevronDown, ChevronUp, Globe, Home, Search, LayoutGrid, Percent, Menu, Package, Briefcase, Save, Users, TrendingUp, Coffee, ToggleLeft, ToggleRight, Gamepad2, ShoppingBag, AlertTriangle, GripVertical, ArrowUp, ArrowDown } from 'lucide-react';
...
type AdminSection = 'homepage' | 'products' | 'orders' | 'computerSets' | 'articles' | 'hero' | 'seo' | 'categoryPages' | 'discounts' | 'careers' | 'headerMenu';
...
const ProductManagement: React.FC<{ products: Product[], categories: Category[], onEdit: (p: Product) => void, onDelete: (id: number) => void }> = ({ products, categories, onEdit, onDelete }) => {
    ...
    const availableBrands = useMemo(() => {
        ...
        if (selectedCategory !== 'all') {
            sourceProducts = products.filter(p => p.category === selectedCategory);
        }
        return [...new Set(sourceProducts.map(p => p.brand).filter(Boolean))].sort();
    ...
    const commonSelectClasses = "block w-full pl-3 pr-10 py-2 border border-gray-300 rounded-md leading-5 bg-white focus:outline-none focus:ring-1 focus:ring-orange-500 focus:border-orange-500 sm:text-sm disabled:bg-gray-100 disabled:cursor-not-allowed";
            ...
            <div className="p-4 bg-gray-50/80 border-b border-gray-200">
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-center">
                    <div className="relative">
                     ...
                     <div>
                        <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className={commonSelectClasses}>
                            <option value="all">ทุกหมวดหมู่</option>
                            {categories.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}
                        </select>
                    </div>
                    <div>
                        <select value={selectedBrand} onChange={(e) => setSelectedBrand(e.target.value)} disabled={availableBrands.length === 0} className={commonSelectClasses}>
                            <option value="all">ทุกแบรนด์</option>
                            {availableBrands.map(brand => <option key={brand} value={brand}>{brand}</option>)}
                        </select>
                    </div>
                    <div>
                         <select value={sortOption} onChange={(e) => setSortOption(e.target.value)} className={commonSelectClasses}>
                ...
                {processedProducts.map(p => (
                    <div key={p.id} className="p-4">
                        <div className="flex items-start gap-4">
                            <img className="h-16 w-16 rounded-md object-cover flex-shrink-0" src={p.imageUrls[0]} alt={p.name} />
            ...
            <div className="overflow-x-auto hidden md:block">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {processedProducts.map(p => (
                            <tr key={p.id}>
                                <td className="px-6 py-4 whitespace-nowrap"><div className="flex items-center"><div className="flex-shrink-0 h-10 w-10"><img className="h-10 w-10 rounded-md object-cover" src={p.imageUrls[0]} alt={p.name} /></div><div className="ml-4"><div className="text-sm font-medium text-gray-900">{p.name}</div><div className="text-sm text-gray-500">{p.category}</div></div></div></td>
...
const ArticleForm: React.FC<{ article: Article | null, onSave: (a: Article) => void, onClose: () => void }> = ({ article, onSave, onClose }) => {
    ...
    return <Modal title={article ? 'แก้ไขบทความ' : 'เพิ่มบทความใหม่'} onClose={onClose}>
        <FormWrapper onSave={handleSave} onCancel={onClose}>
            ...
            <ImageUpload label="รูปภาพประกอบ" currentImages={formState.imageUrl ? [formState.imageUrl] : []} onImagesChange={(imgs) => handleImageChange(imgs[0] || '')} maxImages={1} />
        </FormWrapper>
    </Modal>;
};
...
const HeroSlideForm: React.FC<{ slide: HeroSlide | null, onSave: (s: HeroSlide) => void, onClose: () => void }> = ({ slide, onSave, onClose }) => {
    ...
    return <Modal title={slide ? 'แก้ไขสไลด์' : 'เพิ่มสไลด์ใหม่'} onClose={onClose}>
        <FormWrapper onSave={handleSave} onCancel={onClose}>
            <ImageUpload label="ภาพพื้นหลัง" currentImages={formState.image ? [formState.image] : []} onImagesChange={imgs => handleImageChange(imgs[0] || '')} maxImages={1} />
                        ...
                        {g.products.map(p => (
                            <div key={p.id} className="flex flex-col sm:flex-row justify-between sm:items-center bg-white p-3 border rounded-md gap-2">
                                <div className="flex items-center gap-3">
                                    <img src={p.imageUrls[0]} alt={p.name} className="w-12 h-12 object-contain rounded flex-shrink-0"/>
                                    <div>
...
const ComputerSetGroupForm: React.FC<{ group: ComputerSetGroup | null, onSave: (g: ComputerSetGroup) => void, onClose: () => void }> = ({ group, onSave, onClose }) => {
    ...
    return <Modal title={group?.id ? 'แก้ไขกลุ่มคอมเซ็ต' : 'เพิ่มกลุ่มคอมเซ็ตใหม่'} onClose={onClose}>
        <FormWrapper onSave={handleSave} onCancel={onClose}>
            <ImageUpload label="Banner" currentImages={formState.bannerUrl ? [formState.bannerUrl] : []} onImagesChange={(imgs) => handleImageChange(imgs[0] || '')} maxImages={1} />
        </FormWrapper>
    </Modal>
};
...
const ComputerSetProductForm: React.FC<{ product: ComputerSetProduct | null, onSave: (p: ComputerSetProduct) => void, onClose: () => void }> = ({ product, onSave, onClose }) => {
    const defaultSpecs: ComputerSetSpecs = { cpu: '', vga: '', ram: '', storage: '', motherboard: '', psu: '', case: '', cooling: '', notes: '' };
    ...
    const [formState, setFormState] = useState<Omit<ComputerSetProduct, 'id'>>(initialState);
    
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
        const { name, value } = e.target;
        const isSpecField = Object.keys(defaultSpecs).includes(name);
    ...
    const handleImagesChange = (images: string[]) => setFormState({ ...formState, imageUrls: images });
    const handleSave = () => onSave({ ...formState, id: product?.id || 0 });

    const specFields: { key: keyof ComputerSetSpecs, label: string }[] = [
        { key: 'cpu', label: 'CPU' },
        { key: 'ram', label: 'RAM' },
        { key: 'vga', label: 'VGA (การ์ดจอ)' },
        { key: 'storage', label: 'Storage (SSD/HDD)' },
        { key: 'motherboard', label: 'Motherboard' },
        { key: 'psu', label: 'Power Supply (PSU)' },
        { key: 'case', label: 'Case' },
        { key: 'cooling', label: 'Cooling (ชุดระบายความร้อน)' },
    ];

    return <Modal title={product ? 'แก้ไขสินค้าในเซ็ต' : 'เพิ่มสินค้าในเซ็ต'} onClose={onClose} size="2xl">
        <FormWrapper onSave={handleSave} onCancel={onClose}>
            ...
            <h3 className="text-lg font-semibold text-gray-800 pt-4 border-t mt-4">ส่วนประกอบ (Specs)</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
...
const pageNames: Record<SeoEditablePage, string> = {
    ...
    gamingGear: 'เกมมิ่งเกียร์ (Gaming Gear)',
    desktopComputer: 'คอมพิวเตอร์ตั้งโต๊ะ (Desktop Computer)',
    aioComputer: 'คอมพิวเตอร์ AIO (All-in-one PC)',
    printer: 'ปริ้นเตอร์ (Printer)',
};

const lucideIconNames: LucideIconName[] = [
  'Wrench', 'Computer', 'Monitor', 'Printer', 'Laptop', 'Cpu', 'CircuitBoard',
  'Cog', 'MemoryStick', 'HardDrive', 'Power', 'Box', 'Fan', 'Gamepad2',
  'Router', 'FileCode', 'Server'
];
            ...
            <div className="p-4 sm:p-6 lg:p-8 space-y-8 flex-grow">
                {(Object.keys(categoryConfigurations) as ProductCategory[]).map(pageKey => (
                    <div key={pageKey} className="p-4 border rounded-lg bg-gray-50/80 shadow-sm">
                        ...
                        <ImageUpload label="แบนเนอร์โปรโมชั่น" currentImages={localData[pageKey]?.bannerUrl ? [localData[pageKey]?.bannerUrl] : []} onImagesChange={(base64) => handleBannerChange(pageKey, base64[0] || '')} maxImages={1} />
                    </div>
                ))}
            </div>
            ...
            <div className="space-y-6">
                        ...
                        <table className="min-w-full divide-y divide-gray-200">
                             ...
                             <tbody className="bg-white divide-y divide-gray-200">
                                {order.items.map(item => (
                                    <tr key={item.id}>
                                        <td className="px-4 py-2"><div className="flex items-center gap-3"><img src={item.imageUrls[0]} className="w-10 h-10 object-contain"/><span className="text-sm font-medium">{item.name}</span></div></td>
                ...
                <div className="flex justify-end">
                    <div className="w-full md:w-1/2 lg:w-1/3 space-y-2">
                        ...
                        <div className="flex justify-between text-red-600"><span>ส่วนลด:</span> <span className="text-red-600">- {formatCurrency(order.discount)}</span></div>
                        ...
                        <div className="flex justify-between font-bold text-lg text-gray-800 border-t pt-2 mt-2"><span className="text-gray-800">ยอดรวมสุทธิ:</span> <span className="text-orange-600">{formatCurrency(order.grandTotal)}</span></div>
                    </div>
                </div>
    ...
    const processedOrders = useMemo(() => {
        ...
        if (searchTerm.trim()) {
            ...
            filtered = filtered.filter(o =>
                ...
                o.customer.name.toLowerCase().includes(lowercasedTerm) ||
                o.customer.phone.includes(searchTerm) // phone number search doesn't need to be lowercased
            );
        }
            ...
            <div className="p-4 bg-gray-50/80 border-b border-gray-200">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="relative">
                        ...
                        <input
                            type="text"
                            placeholder="ค้นหา Order #, ชื่อลูกค้า, เบอร์โทร..."
                            value={searchTerm}
                    ...
                    <div key={order.id} className="p-4">
                        <div className="flex justify-between items-start">
                            <div>
                                ...
                                <p className="text-xs text-gray-500">{new Date(order.date).toLocaleDateString('th-TH')}</p>
                            </div>
                        ...
                        <div className="mt-3 flex justify-between items-center">
                            <p className="font-semibold text-orange-600">{formatCurrency(order.grandTotal)}</p>
                            <button onClick={() => handleViewDetails(order)} className="text-sm font-semibold text-indigo-600 hover:text-indigo-900">
                                ดูรายละเอียด
                            </button>
                        </div>
                    </div>
                )) : (
                     <div className="text-center p-8 text-gray-500">ไม่พบรายการคำสั่งซื้อ</div>
                )}
            </div>
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {processedOrders.length > 0 ? processedOrders.map(order => (
                            <tr key={order.id}>
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{formatCurrency(order.grandTotal)}</td>
                            ...
                            <tr>
                                <td colSpan={6} className="text-center p-8 text-gray-500">
                                    ไม่พบรายการคำสั่งซื้อที่ตรงกับเกณฑ์การค้นหา
                                </td>
                            </tr>
                        )}
                    </tbody>
...
const CareersManagement: React.FC<{ content: CareersPageContent, onSave: (newContent: CareersPageContent) => void }> = ({ content, onSave }) => {
    ...
    return (
        <ManagementPanel title="จัดการหน้าสมัครงาน (Careers)">
            <div className="flex flex-col">
                <div className="p-4 sm:p-6 lg:p-8 space-y-6 flex-grow">
                    <SectionEditor title="Hero Section">
                        <FormInput label="Title" name="hero-title" value={localContent.hero.title} onChange={(e) => handleSimpleChange('hero', 'title', e.target.value)} />
                        <FormInput label="Subtitle" name="hero-subtitle" value={localContent.hero.subtitle} onChange={(e) => handleSimpleChange('hero', 'subtitle', e.target.value)} />
                    </SectionEditor>

                    <SectionEditor title="Why Us Section">
                        <FormInput label="Title" name="whyUs-title" value={localContent.whyUs.title} onChange={(e) => handleSimpleChange('whyUs', 'title', e.target.value)} />
                        <FormInput label="Subtitle" name="whyUs-subtitle" value={localContent.whyUs.subtitle} onChange={(e) => handleSimpleChange('whyUs', 'subtitle', e.target.value)} />
                        <h4 className="font-semibold mt-4">Features</h4>
                        {localContent.whyUs.features.map((feature) => (
                        <div key={feature.id} className="p-3 border rounded-md relative bg-gray-50/50">
                            <p className="font-medium">{feature.title}</p>
                            <p className="text-sm text-gray-600">{feature.description}</p>
                            <div className="absolute top-2 right-2 flex gap-2">
                            <button onClick={() => { setEditingFeature(feature); setIsFeatureModalOpen(true); }} className="p-1 text-indigo-600 hover:bg-indigo-100 rounded-full"><Edit size={16}/></button>
                            <button onClick={() => {
                                if (window.confirm('Are you sure?')) {
                                setLocalContent(prev => ({...prev, whyUs: {...prev.whyUs, features: prev.whyUs.features.filter(f => f.id !== feature.id)}}));
                                }
                            }} className="p-1 text-red-600 hover:bg-red-100 rounded-full"><Trash2 size={16}/></button>
                            </div>
                        </div>
                        ))}
                        <button onClick={() => { setEditingFeature(null); setIsFeatureModalOpen(true); }} className="mt-2 flex items-center bg-blue-500 text-white text-sm font-bold py-1.5 px-3 rounded-md hover:bg-blue-600"><PlusCircle size={16} className="mr-2"/>เพิ่ม Feature</button>
                    </SectionEditor>

                    <SectionEditor title="Job Openings Section">
                        <FormInput label="Title" name="openings-title" value={localContent.openings.title} onChange={(e) => handleSimpleChange('openings', 'title', e.target.value)} />
                        <FormInput label="Subtitle" name="openings-subtitle" value={localContent.openings.subtitle} onChange={(e) => handleSimpleChange('openings', 'subtitle', e.target.value)} />
                        <h4 className="font-semibold mt-4">ตำแหน่งงาน</h4>
                        {localContent.openings.jobs.map((job) => (
                            <div key={job.id} className="p-3 border rounded-md relative bg-gray-50/50">
                                <p className="font-medium">{job.title} <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${job.isActive ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800'}`}>{job.isActive ? 'เปิดรับ' : 'ปิดรับ'}</span></p>
                                <p className="text-sm text-gray-600">{job.location} - {job.type}</p>
                                <div className="absolute top-2 right-2 flex gap-2">
                                    <button onClick={() => { setEditingJob(job); setIsJobModalOpen(true); }} className="p-1 text-indigo-600 hover:bg-indigo-100 rounded-full"><Edit size={16}/></button>
                                    <button onClick={() => {
                                        if (window.confirm('คุณแน่ใจหรือไม่ว่าต้องการลบตำแหน่งงานนี้?')) {
                                            setLocalContent(prev => ({...prev, openings: {...prev.openings, jobs: prev.openings.jobs.filter(j => j.id !== job.id)}}));
                                        }
                                    }} className="p-1 text-red-600 hover:bg-red-100 rounded-full"><Trash2 size={16}/></button>
                                </div>
                            </div>
                        ))}
                        <button onClick={() => { setEditingJob(null); setIsJobModalOpen(true); }} className="mt-2 flex items-center bg-blue-500 text-white text-sm font-bold py-1.5 px-3 rounded-md hover:bg-blue-600"><PlusCircle size={16} className="mr-2"/>เพิ่มตำแหน่งงาน</button>
                    </SectionEditor>

                    <SectionEditor title="Call to Action Section">
                        <FormInput label="Title" name="cta-title" value={localContent.cta.title} onChange={(e) => handleSimpleChange('cta', 'title', e.target.value)} />
                        <FormInput label="Subtitle" name="cta-subtitle" value={localContent.cta.subtitle} onChange={(e) => handleSimpleChange('cta', 'subtitle', e.target.value)} />
                        <FormInput label="Email" name="cta-email" value={localContent.cta.email} onChange={(e) => handleSimpleChange('cta', 'email', e.target.value)} />
                    </SectionEditor>
                </div>
                <div className="p-4 sm:p-6 lg:p-8 border-t bg-white/80 backdrop-blur-sm sticky bottom-0 flex justify-end">
                    <button onClick={handleSave} className="bg-green-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-green-700 transition-colors">บันทึกหน้าสมัครงาน</button>
                </div>
            </div>
            {isFeatureModalOpen && <FeatureCardForm feature={editingFeature} onClose={() => setIsFeatureModalOpen(false)} onSave={(feature) => {
            ...
            {isJobModalOpen && <JobOpeningForm job={editingJob} onClose={() => setIsJobModalOpen(false)} onSave={(job) => {
                const newJobs = editingJob
                    ? localContent.openings.jobs.map(j => j.id === job.id ? job : j)
                    : [...localContent.openings.jobs, job];
                setLocalContent(prev => ({...prev, openings: {...prev.openings, jobs: newJobs}}));
...
const HeaderMenuManagement: React.FC<{ data: HeaderMenuData; onSave: (newData: HeaderMenuData) => void; }> = ({ data, onSave }) => {
  const [localData, setLocalData] = useState<HeaderMenuData>(data);
  const [editingLink, setEditingLink] = useState<{ groupIndex: number, link: CategoryLink | null } | null>(null);
  const { addToast } = useToast();
  ...
  const handleAddGroup = () => {
    ...
    setLocalData([...localData, newGroup]);
  };
  
  const handleUpdateGroupTitle = (groupId: number, newTitle: string) => {
      setLocalData(localData.map(g => g.id === groupId ? {...g, title: newTitle} : g));
  };
  
  const handleDeleteGroup = (groupId: number) => {
      if (window.confirm('คุณแน่ใจหรือไม่ว่าต้องการลบกลุ่มนี้และลิงก์ทั้งหมดในกลุ่ม?')) {
          setLocalData(localData.filter(g => g.id !== groupId));
      }
  };
  ...
  // Link functions
  const handleSaveLink = (groupIndex: number, linkToSave: CategoryLink) => {
      const group = localData[groupIndex];
      const exists = group.links.some(l => l.id === linkToSave.id);
      const newLinks = exists
        ? group.links.map(l => l.id === linkToSave.id ? linkToSave : l)
        : [...group.links, { ...linkToSave, id: Date.now() }];
      
      const newGroups = [...localData];
      newGroups[groupIndex] = { ...group, links: newLinks };
      setLocalData(newGroups);
      setEditingLink(null);
  };
  
  const handleDeleteLink = (groupIndex: number, linkId: number) => {
      if (window.confirm('คุณแน่ใจหรือไม่ว่าต้องการลบลิงก์นี้?')) {
          const group = localData[groupIndex];
          const newLinks = group.links.filter(l => l.id !== linkId);
          const newGroups = [...localData];
          newGroups[groupIndex] = { ...group, links: newLinks };
          setLocalData(newGroups);
      }
  ...
  const moveLink = (groupIndex: number, linkIndex: number, direction: 'up' | 'down') => {
      ...
      [newLinks[linkIndex], newLinks[newIndex]] = [newLinks[newIndex], newLinks[linkIndex]]; // Swap
  ...
  const LinkFormModal: React.FC<{
      ...
      onSave: (groupIndex: number, link: CategoryLink) => void;
      onClose: () => void;
  }> = ({ groupIndex, link, onSave, onClose }) => {
      const [formState, setFormState] = useState<Omit<CategoryLink, 'id'>>({
          name: '',
          icon: 'Computer',
          page: 'home',
          ...link
      ...
      const handleSaveClick = () => {
          onSave(groupIndex, { ...formState, id: link?.id || 0 });
      };

      return (
          <Modal title={link ? 'แก้ไขลิงก์' : 'เพิ่มลิงก์ใหม่'} onClose={onClose}>
              <FormWrapper onSave={handleSaveClick} onCancel={onClose}>
                  <FormInput label="ชื่อลิงก์" name="name" value={formState.name} onChange={handleChange} />
                  <div>
                      <label htmlFor="icon" className="block text-sm font-medium text-gray-700">ไอคอน</label>
                      <select name="icon" id="icon" value={formState.icon} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500">
                  ...
                  <div>
                      <label htmlFor="page" className="block text-sm font-medium text-gray-700">หน้าปลายทาง</label>
                      <select name="page" id="page" value={formState.page} onChange={handleChange} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-orange-500 focus:border-orange-500">
                          {Object.entries(pageNames).map(([pageKey, pageName]) => (
                              <option key={pageKey} value={pageKey}>{pageName}</option>
                          ))}
                      </select>
                  </div>
              </FormWrapper>
  ...
  return (
    <ManagementPanel title="จัดการเมนูหลัก (Header)">
      <div className="flex flex-col">
        <div className="p-4 sm:p-6 lg:p-8 space-y-4 flex-grow">
            ...
            <div key={group.id} className="border rounded-lg bg-white shadow-sm">
              <div className="p-3 bg-gray-50 flex items-center justify-between">
                <input 
                  ...
                  className="font-bold text-lg text-gray-800 bg-transparent border-none focus:ring-2 focus:ring-orange-300 rounded-md p-1 -m-1"
                />
                <div className="flex items-center gap-2">
                    <button onClick={() => moveGroup(groupIndex, 'up')} disabled={groupIndex === 0} className="p-1.5 rounded-full hover:bg-gray-200 disabled:opacity-30"><ArrowUp size={16}/></button>
                    <button onClick={() => moveGroup(groupIndex, 'down')} disabled={groupIndex === localData.length - 1} className="p-1.5 rounded-full hover:bg-gray-200 disabled:opacity-30"><ArrowDown size={16}/></button>
                    <button onClick={() => handleDeleteGroup(group.id)} className="p-1.5 text-red-600 rounded-full hover:bg-red-100"><Trash2 size={16}/></button>
                </div>
              </div>
              <div className="p-4 space-y-2">
                {group.links.map((link, linkIndex) => (
                    <div key={link.id} className="flex items-center justify-between p-2 border rounded-md bg-gray-50/50 hover:bg-gray-100">
                      <span className="text-sm font-medium">{link.name} <span className="text-xs text-gray-500">({pageNames[link.page as SeoEditablePage]})</span></span>
                      <div className="flex items-center gap-2">
                          <button onClick={() => moveLink(groupIndex, linkIndex, 'up')} disabled={linkIndex === 0} className="p-1.5 rounded-full hover:bg-gray-200 disabled:opacity-30"><ArrowUp size={14}/></button>
                          <button onClick={() => moveLink(groupIndex, linkIndex, 'down')} disabled={linkIndex === group.links.length - 1} className="p-1.5 rounded-full hover:bg-gray-200 disabled:opacity-30"><ArrowDown size={14}/></button>
                          <button onClick={() => setEditingLink({ groupIndex, link })} className="p-1.5 text-indigo-600 rounded-full hover:bg-indigo-100"><Edit size={14}/></button>
                          <button onClick={() => handleDeleteLink(groupIndex, link.id)} className="p-1.5 text-red-600 rounded-full hover:bg-red-100"><Trash2 size={14}/></button>
                      </div>
                    </div>
                ))}
                <button onClick={() => setEditingLink({ groupIndex, link: null })} className="mt-2 text-sm font-semibold text-blue-600 hover:text-blue-500 flex items-center gap-1">
                  <PlusCircle size={14}/> เพิ่มลิงก์
                </button>
              </div>
            </div>
          ))}
          <button onClick={handleAddGroup} className="w-full text-center py-3 border-2 border-dashed rounded-lg text-gray-600 hover:border-orange-500 hover:text-orange-600 transition-colors font-semibold">
            + เพิ่มกลุ่มใหม่
          </button>
        </div>
        <div className="p-4 sm:p-6 lg:p-8 border-t bg-white/80 backdrop-blur-sm sticky bottom-0 flex justify-end items-center gap-4">
          <button onClick={handleSave} className="bg-green-600 text-white font-bold py-2 px-6 rounded-lg hover:bg-green-700 transition-colors flex items-center gap-2">
            <Save size={18} />
            บันทึกการเปลี่ยนแปลงเมนู
          </button>
        </div>
      </div>
      {editingLink && (
        <LinkFormModal 
          groupIndex={editingLink.groupIndex}
...
export const AdminPage: React.FC<AdminPageProps> = (props) => {
  const {
    onLogout, onResetAllData,
    products, categories, onSaveProduct, onDeleteProduct,
    articles, onSaveArticle, onDeleteArticle,
    heroSlides, onSaveHeroSlide, onDeleteHeroSlide,
    computerSetGroups, onSaveComputerSetGroup, onDeleteComputerSetGroup,
    seoData, onSaveSeoData,
    homepageContent, onSaveHomepageContent,
    categoryPageData, onSaveCategoryPageData,
    discountCodes, onSaveDiscountCode, onDeleteDiscountCode,
    adminOrders, onSaveAdminOrder, onDeleteAdminOrder,
    careersPageContent, onSaveCareersPageContent,
    headerMenuData, onSaveHeaderMenuData
  } = props;
  ...
  const handleSaveAndCloseProduct = (product: Product) => {
    onSaveProduct(product);
    setIsProductFormOpen(false);
    addToast(editingProduct ? 'อัปเดตสินค้าเรียบร้อยแล้ว' : 'เพิ่มสินค้าใหม่เรียบร้อยแล้ว', 'success');
  };
  
  const navItems = [
    { id: 'homepage', label: 'จัดการหน้าแรก', icon: <Home size={18} /> },
    { id: 'products', label: 'จัดการสินค้า', icon: <ShoppingBag size={18} /> },
    { id: 'orders', label: 'รายการสั่งซื้อ', icon: <ShoppingCart size={18} /> },
    { id: 'computerSets', label: 'จัดการคอมเซ็ต', icon: <Tv size={18} /> },
    { id: 'articles', label: 'จัดการบทความ', icon: <Newspaper size={18} /> },
    { id: 'hero', label: 'จัดการ Hero Banner', icon: <ImageIcon size={18} /> },
    { id: 'categoryPages', label: 'จัดการหน้าหมวดหมู่', icon: <LayoutGrid size={18} /> },
    { id: 'discounts', label: 'จัดการโค้ดส่วนลด', icon: <Percent size={18} /> },
    { id: 'headerMenu', label: 'จัดการเมนู', icon: <Menu size={18} /> },
    { id: 'careers', label: 'จัดการหน้าสมัครงาน', icon: <Briefcase size={18} /> },
    { id: 'seo', label: 'จัดการ SEO', icon: <Globe size={18} /> },
  ];
  
  const renderSection = () => {
    switch (activeSection) {
      case 'homepage':
        return <HomepageManagement content={homepageContent} onSave={onSaveHomepageContent} />;
      case 'products':
        return (
          <ManagementPanel title="สินค้าทั้งหมด" onAddNew={handleAddNewProduct}>
            <ProductManagement products={products} categories={categories} onEdit={handleEditProduct} onDelete={onDeleteProduct} />
          </ManagementPanel>
        );
      case 'orders':
        return <OrderManagement orders={adminOrders} onSave={onSaveAdminOrder} onDelete={onDeleteAdminOrder} />;
      case 'computerSets':
        return <ComputerSetManagement groups={computerSetGroups} onSave={onSaveComputerSetGroup} onDelete={onDeleteComputerSetGroup} />;
      case 'articles':
        return <ArticleManagement articles={articles} onSave={onSaveArticle} onDelete={onDeleteArticle} />;
      case 'hero':
        return <HeroSlideManagement slides={heroSlides} onSave={onSaveHeroSlide} onDelete={onDeleteHeroSlide} />;
      case 'seo':
        return <SeoManagement seoData={seoData} onSave={onSaveSeoData} />;
      case 'categoryPages':
        return <CategoryPageManagement data={categoryPageData} onSave={onSaveCategoryPageData} />;
      case 'discounts':
        return <DiscountManagement codes={discountCodes} onSave={onSaveDiscountCode} onDelete={onDeleteDiscountCode} />;
      case 'careers':
        return <CareersManagement content={careersPageContent} onSave={onSaveCareersPageContent} />;
      case 'headerMenu':
        return <HeaderMenuManagement data={headerMenuData} onSave={onSaveHeaderMenuData} />;
      default:
        return <div className="p-8">กรุณาเลือกเมนู</div>;
    }
  };
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 font-sans">
      <aside className="w-64 bg-gray-800 text-white flex flex-col flex-shrink-0">
        <div className="h-20 flex items-center justify-center text-2xl font-bold border-b border-gray-700">ADMIN</div>
        <nav className="flex-grow p-4 space-y-2">
          {navItems.map(item => (
            <button key={item.id} onClick={() => setActiveSection(item.id as AdminSection)} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium transition-colors ${activeSection === item.id ? 'bg-orange-500 text-white' : 'hover:bg-gray-700'}`}>
              {item.icon} {item.label}
            </button>
          ))}
        </nav>
        <div className="p-4 border-t border-gray-700 space-y-2">
          <button onClick={onResetAllData} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium bg-red-800 hover:bg-red-700">
              <AlertTriangle size={18} /> รีเซ็ตข้อมูลทั้งหมด
          </button>
          <button onClick={onLogout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-sm font-medium bg-gray-700 hover:bg-gray-600">
      ...
      {isProductFormOpen && (
        <ProductForm 
          ...
          onSave={handleSaveAndCloseProduct} 
