...
import { Product, Category, Article, ComputerSetGroup, HeroSlide, Page, SeoData, SeoEditablePage, Brand, DisplayProduct, HomepageContent, ProductCategory, AllCategoryPageData, DiscountCode, AdminOrder, CareersPageContent, ComputerSetProduct, Review, HeaderMenuData } from './types';
import { mockProducts, mockCategories, mockComputerSetGroups, mockArticles, mockHeroSlides, mockSeoData, mockHomepageContent, mockCategoryPageData, mockDiscountCodes, mockAdminOrders, mockCareersPageContent, mockHeaderMenuData } from './constants';
...
const App: React.FC = () => {
  ...
  // Admin and Data State - Now powered by localStorage
  ...
  const handleSaveArticle = (articleToSave: Article) => {
    setArticles(prev => {
        ...
        const newArticles = exists
            ...
            : [...prev, { ...articleToSave, id: Math.max(0, ...prev.map(a => a.id)) + 1, link: `/article/${Math.max(0, ...prev.map(a => a.id)) + 1}` }];
    ...
    setComputerSetGroups(prev => {
        ...
        const newGroups = exists
            ...
            : [...prev, {
                ...
                products: groupToSave.products.map((p, index) => ({ ...p, id: p.id || (Date.now() + index) }))
              }];
  ...
  const handleSaveCareersPageContent = (newContent: CareersPageContent) => {
    saveData('careersPageContent', newContent);
    setCareersPageContent(newContent);
  };
  
  const handleSaveHeaderMenuData = (newData: HeaderMenuData) => {
      saveData('headerMenuData', newData);
      setHeaderMenuData(newData);
  };
  ...
  const handleSaveReview = (productId: number, reviewData: Omit<Review, 'id' | 'productId' | 'date'>) => {
    setProducts(prevProducts => {
        ...
        // Also update the main rating and reviewCount for consistency
        ...
        saveData('products', newProducts);
        // also update the current product if it's being viewed
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      case 'home':
        return (
          <>
            ...
            <div className="animate-slide-in-up" style={{ animationDelay: '100ms' }}><ProductDisplaySection viewAllPage="allProducts" onNavigate={navigateTo} title={homepageContent.sectionTitles.newProducts} products={homepageContent.newProducts} brands={[]} sidebarTitle="สินค้าใหม่" /></div>
      ...
      case 'productDetail':
        return currentProduct ? <ProductPage product={currentProduct} allProducts={products} onNavigate={navigateTo} onSaveReview={handleSaveReview} /> : null;
      case 'searchResults':
        return <SearchResultsPage query={searchQuery} allProducts={products} onNavigate={navigateTo} />;
      case 'allProducts':
        return <AllProductsPage allProducts={products} onNavigate={navigateTo} />;
      case 'promotions':
      ...
      case 'reviews':
        return <ReviewsPage products={products} onNavigate={navigateTo} />;
      case 'privacy':
