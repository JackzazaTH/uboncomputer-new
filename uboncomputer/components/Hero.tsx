    ...
    <section className="relative h-[550px] md:h-[650px] 2xl:h-[750px] w-full overflow-hidden bg-gray-900 group">
      {slides.map((slide, index) => (
         <div key={slide.id} className={`absolute inset-0 w-full h-full transition-opacity duration-1000 ease-in-out ${index === currentIndex ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
            ...
            <div className="absolute inset-0 bg-gradient-to-r from-black/80 via-black/20 to-transparent opacity-70"></div>
         </div>
      ))}
        ...
        <div className="flex flex-col justify-center h-full text-white w-full md:w-3/4 lg:w-2/3 xl:w-1/2 pb-12 md:pb-0 pt-16">
            <div className={`space-y-6 transition-all duration-700 transform ${currentIndex >= 0 ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
              <h1 className="text-4xl sm:text-5xl md:text-6xl 2xl:text-7xl font-black tracking-tight leading-[1.1] drop-shadow-2xl">
                  <span className="block text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-300 filter drop-shadow-lg animate-fade-in">{slides[currentIndex].titleLine1}</span>
                  <span className="block text-orange-500 filter drop-shadow-lg mt-2">{slides[currentIndex].titleLine2}</span>
                  <span className="block text-gray-200 text-2xl sm:text-3xl md:text-4xl mt-4 font-bold shadow-black font-heading tracking-wider uppercase">{slides[currentIndex].titleLine3}</span>
              </h1>
             <div className="mt-10 sm:mt-12 flex flex-wrap gap-4">
               <a
                href="#products"
                className="inline-flex items-center bg-gradient-to-r from-orange-600 to-red-600 text-white font-bold text-base sm:text-lg px-10 py-4 rounded-full hover:shadow-[0_10px_25px_-5px_rgba(234,88,12,0.5)] transition-all transform hover:-translate-y-1 active:scale-95 ring-1 ring-white/20 backdrop-blur-sm"
              >
