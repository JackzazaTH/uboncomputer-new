...
import { Product, Category, Article, ComputerSetGroup, HeroSlide, Page, SeoData, HomepageContent, AllCategoryPageData, DiscountCode, NavigateFunction } from '../types';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const onNavigate: NavigateFunction = (page, data) => {
    ...
    if (page === 'articleDetail' && data?.articleId) {
        path = `/article/${data.articleId}`;
    } else if (page === 'productDetail' && data?.productId) {
        path = `/product/${data.productId}`;
    } else if (page === 'searchResults' && data?.searchQuery) {
        path = `/search?q=${encodeURIComponent(data.searchQuery)}`;
    } else {
        path = paths[page] || '/';
    }
    router.push(path);
