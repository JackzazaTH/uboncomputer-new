...
import { HomepageContent, PromoBanner, YoutubeVideo, StoreBranch, InfoBarItem, DualCarouselContent, DisplayProduct, Brand } from '../types';
...
const DisplayProductForm: React.FC<{ product: DisplayProduct | null, onSave: (p: DisplayProduct) => void, onClose: () => void }> = ({ product, onSave, onClose }) => {
    const [formState, setFormState] = useState<Omit<DisplayProduct, 'id'>>({ name: '', specs: '', price: 0, oldPrice: 0, imageUrl: '', brand: '', discountPercentage: 0, ...product });
             ...
             {brands.map((brand, index) => (
                 <div key={brand.id} className="flex items-end gap-3 p-3 border rounded-md bg-white">
                     <div className="flex-grow grid grid-cols-1 md:grid-cols-2 gap-3 items-center">
                         <ImageUpload label={`โลโก้ ${index+1}`} currentImage={brand.logoUrl} onImageChange={base64 => handleUpdate(index, 'logoUrl', base64)} />
...
const HomepageManagement: React.FC<HomepageManagementProps> = ({ content, onSave }) => {
  ...
  const handleListChange = (section: 'promoBanners' | 'youtube' | 'storeInfo' | 'infoBar', listName: 'videos' | 'branches' | 'items' | null, index: number, field: string, value: any) => {
      setLocalContent(prev => {
  ...
  const addListItem = (section: 'promoBanners' | 'youtube' | 'storeInfo' | 'infoBar', listName: 'videos' | 'branches' | 'items' | null) => {
    setLocalContent(prev => {
        const listKey = listName ? listName : section;
        // @ts-ignore
        const currentList = listName ? prev[section][listKey] : prev[section];
  ...
  const removeListItem = (section: 'promoBanners' | 'youtube' | 'storeInfo' | 'infoBar', listName: 'videos' | 'branches' | 'items' | null, index: number) => {
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-6">
        <div className="flex justify-between items-center mb-6 sticky top-0 bg-gray-100/80 backdrop-blur-sm py-4 z-10 -mx-8 px-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-800">จัดการหน้าแรก</h2>
      ...
      <SectionEditor title="ข้อความแนะนำร้าน">
          ...
          {localContent.promoBanners.map((banner, index) => (
              <div key={banner.id} className="p-4 border rounded-lg space-y-3 relative">
                  <h4 className="font-semibold">แบนเนอร์ {index + 1}</h4>
                   <ImageUpload label="รูปภาพ" currentImage={banner.src} onImageChange={base64 => handleListChange('promoBanners', null, index, 'src', base64)} />
                    ...
                    <div key={slide.id} className="p-3 border rounded-md relative bg-gray-50/50">
                        ...
                        <div className="space-y-3">
                            <ImageUpload
                                ...
                                onImageChange={base64 => handleCarouselChange(carousel.id, 'update', slideIndex, 'src', base64)}
                            />
                            <FormInput
                                label="Alt Text"
      ...
      <SectionEditor title="ข้อมูลร้านและสาขา">
        <ImageUpload label="รูปภาพหลักของร้าน" currentImage={localContent.storeInfo.mainImage} onImageChange={base64 => handleSimpleChange('storeInfo', 'mainImage', base64)} />
      ...
      <SectionEditor title="แบนเนอร์โปรโมชั่นหลัก (เต็มความกว้าง)">
          <ImageUpload label="รูปภาพ" currentImage={localContent.mainPromoBanner.src} onImageChange={base64 => handleSimpleChange('mainPromoBanner', 'src', base64)} />
