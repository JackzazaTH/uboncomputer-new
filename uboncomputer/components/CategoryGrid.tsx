                    ...
                    {categories.map((category) => {
                        return (
                            <button 
                                ...
                                className="flex flex-col items-center justify-center p-4 bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:border-orange-200 hover:-translate-y-1 transition-all duration-300 group h-full"
                            >
                                <div className={`p-3 rounded-2xl transition-colors mb-3 ${category.color} group-hover:scale-110 duration-300`}>
                                    {React.isValidElement(category.icon) ? React.cloneElement(category.icon as React.ReactElement<any>, { size: 28, strokeWidth: 1.5 }) : null}
                                </div>
                                <span className="text-xs font-bold text-gray-700 group-hover:text-orange-600 text-center transition-colors">{category.name}</span>
                            </button>
                        );
                    })}
                </div>
