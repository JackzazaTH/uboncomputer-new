...
const Header: React.FC<HeaderProps> = ({ onSearchSubmit, onNavigate, onAuthClick, currentPage, topBarText, allProducts, headerMenuData, isUserLoggedIn, currentUser, onUserLogout, branding }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-[900] transition-all duration-300 overflow-visible ${isScrolled ? 'bg-white/95 backdrop-blur-md border-b border-gray-200/50 shadow-sm py-2' : 'bg-white border-b border-gray-100 py-3'}`}>
      {/* Top Bar */}
      <div className={`bg-slate-900 text-white transition-all duration-500 overflow-hidden ${isScrolled ? 'h-0' : 'h-8'}`}>
      ...
      <div className="container mx-auto px-4 sm:px-6 2xl:px-8">
        <div className="flex justify-between items-center h-16 transition-all duration-300">
          ...
          {/* Search Bar - Desktop */}
          <div ref={searchContainerRef} className="hidden 2xl:flex flex-1 justify-center max-w-2xl mx-auto px-4">
            <form onSubmit={handleSearchSubmit} className="w-full relative group">
              ...
              {isPreviewOpen && liveSearchTerm && (
                 <div className="absolute top-full left-0 right-0 mt-3 rounded-2xl shadow-xl overflow-hidden bg-white/95 backdrop-blur-xl border border-gray-100 animate-fade-in-up ring-1 ring-black/5 z-[950]">
           ...
           <div className="flex items-center h-full relative">
                ...
                <div className="relative mr-6" ref={categoryDropdownRef}>
                    ...
                    {isCategoryDropdownOpen && (
                        <div className="absolute top-full left-0 mt-4 w-[900px] bg-white/98 backdrop-blur-2xl rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] border border-gray-100 z-[910] overflow-hidden flex animate-fade-in-up origin-top-left ring-1 ring-black/5">
                            {/* Sidebar Groups */}
      ...
      {isMobileMenuOpen && (
        <>
            <div className="2xl:hidden fixed inset-0 bg-black/60 backdrop-blur-sm z-[900]" onClick={() => setIsMobileMenuOpen(false)}></div>
