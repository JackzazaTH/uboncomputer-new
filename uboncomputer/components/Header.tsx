...
const Header: React.FC<HeaderProps> = ({ onSearchSubmit, onNavigate, onAuthClick, currentPage, topBarText, allProducts, headerMenuData, isUserLoggedIn, currentUser, onUserLogout, branding }) => {
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? 'bg-white/90 backdrop-blur-md border-b border-gray-100 shadow-sm py-2' : 'bg-white border-b border-gray-50 py-4'}`}>
      {/* Top Bar */}
      ...
      <div className="container mx-auto px-4 sm:px-6 2xl:px-8">
        <div className="flex justify-between items-center h-16 transition-all duration-300">
          ...
          {/* Search Bar - Desktop */}
          <div ref={searchContainerRef} className="hidden 2xl:flex flex-1 justify-center max-w-2xl mx-auto">
            <form onSubmit={handleSearchSubmit} className="w-full relative group">
              ...
              <input 
                ...
                className="w-full bg-gray-100 border border-transparent text-gray-900 text-sm rounded-full focus:ring-2 focus:ring-orange-500/20 focus:bg-white focus:border-orange-500 block pl-10 pr-12 py-2.5 transition-all duration-300 placeholder:text-gray-400 hover:bg-white hover:shadow-sm hover:border-gray-200"
              />
              ...
              {isPreviewOpen && liveSearchTerm && (
                 <div className="absolute top-full left-0 right-0 mt-3 rounded-2xl shadow-xl overflow-hidden bg-white/80 backdrop-blur-xl border border-white/20 animate-fade-in-up ring-1 ring-black/5">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 ml-auto">
            ...
            <div className="hidden 2xl:flex items-center gap-2">
              {isUserLoggedIn && currentUser ? (
                ...
                <button onClick={onAuthClick} className="flex items-center gap-2 text-sm font-bold text-gray-600 hover:text-orange-600 transition-colors px-3 py-2 rounded-full hover:bg-orange-50">
              ...
              <button onClick={() => onNavigate('compare')} className="relative p-2.5 text-gray-500 hover:text-orange-600 transition-colors group rounded-full hover:bg-gray-50">
              ...
              <button onClick={() => onNavigate('cart')} className="relative p-2.5 text-gray-500 hover:text-orange-600 transition-colors group rounded-full hover:bg-gray-50">
        ...
        <div className={`hidden 2xl:block transition-all duration-500 ease-in-out overflow-hidden ${isScrolled ? 'h-0 opacity-0 mt-0' : 'h-12 opacity-100 border-t border-gray-50 mt-2'}`}>
           <div className="flex items-center h-full">
                {/* Category Dropdown */}
                <div className="relative mr-6" ref={categoryDropdownRef}>
                    <button 
                        onClick={() => setIsCategoryDropdownOpen(!isCategoryDropdownOpen)} 
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-sm font-bold transition-all duration-300 ${isCategoryDropdownOpen ? 'bg-gray-900 text-white shadow-md transform scale-105' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                    >
                        <LayoutGrid size={16} />
                    ...
                    {isCategoryDropdownOpen && (
                        <div className="absolute top-full left-0 mt-4 w-[850px] bg-white/95 backdrop-blur-xl rounded-3xl shadow-2xl border border-gray-100 z-50 overflow-hidden flex animate-fade-in-up origin-top-left ring-1 ring-black/5">
                            {/* Sidebar Groups */}
                            <div className="w-72 bg-gray-50/80 border-r border-gray-100 p-3 space-y-1">
                            ...
                            <div className="flex-1 p-8 bg-white">
                                {headerMenuData.map((group) => (
                                    expandedGroups.includes(group.id) && (
                                        <div key={group.id} className="grid grid-cols-2 gap-x-6 gap-y-4 animate-fade-in">
                                            {group.links.map((link) => (
                                                <button 
                                                    key={link.id} 
                                                    onClick={() => { onNavigate(link.page); setIsCategoryDropdownOpen(false); }} 
                                                    className="flex items-center gap-4 p-3 rounded-2xl hover:bg-gray-50 group transition-all duration-200 border border-transparent hover:border-gray-100 hover:shadow-sm"
                                                >
                                                    <div className="w-10 h-10 rounded-xl bg-orange-50 group-hover:bg-white flex items-center justify-center text-orange-500 group-hover:text-orange-600 transition-all shadow-sm group-hover:shadow-md ring-1 ring-orange-100/50">
                                                        {iconMap[link.icon]}
                                                    </div>
                                                    <div className="text-left">
                                                        <span className="block text-sm font-bold text-gray-800 group-hover:text-orange-700 transition-colors">{link.name}</span>
                                                        <span className="block text-[10px] text-gray-400 font-medium group-hover:text-gray-500 mt-0.5">เลือกซื้อ {link.name}</span>
                                                    </div>
                                                </button>
                                            ))}
                                        </div>
                ...
                <nav className="flex items-center gap-1 flex-1 overflow-x-auto no-scrollbar">
                    {mainNavLinks.map((link) => { 
                        ...
                        const baseClasses = "flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold transition-all duration-300 whitespace-nowrap select-none";
