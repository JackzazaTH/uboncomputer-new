                    ...
                    {categories.map((category) => {
                        return (
                            <button 
                                ...
                                className={`flex flex-col items-center justify-center p-4 rounded-2xl hover:shadow-lg hover:-translate-y-1 transition-all duration-300 group h-full border ${category.border} ${category.bg} hover:bg-white relative overflow-hidden`}
                            >
                                <div className={`w-12 h-12 rounded-full bg-white flex items-center justify-center shadow-sm mb-3 group-hover:scale-110 transition-transform duration-300 z-10`}>
                                    {React.isValidElement(category.icon) ? React.cloneElement(category.icon as React.ReactElement<any>, { size: 24, className: category.color, strokeWidth: 2 }) : null}
                                </div>
                                <span className="text-xs font-bold text-gray-700 group-hover:text-gray-900 text-center transition-colors line-clamp-1 w-full z-10">{category.name}</span>
