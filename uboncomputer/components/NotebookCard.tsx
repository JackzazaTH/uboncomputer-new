...
import { DisplayProduct, Product, Page, NavigateFunction } from '../types';
...
const NotebookCard: React.FC<NotebookCardProps> = ({ product, onNavigate }) => {
  ...
  return (
    <a 
      ...
      className="bg-white rounded-lg overflow-hidden flex flex-col group border border-gray-200 hover:shadow-lg transition-all duration-300 h-full p-4 relative"
    >
      ...
      <div className="relative mb-4">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent flex items-end justify-center gap-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10 pb-4">
            <a href="#" onClick={(e) => { e.stopPropagation(); onNavigate('productDetail', { productId: product.id }); }} className="p-3 bg-white/90 rounded-full text-gray-800 hover:bg-white hover:text-orange-500 transition-all scale-90 group-hover:scale-100" title="ดูรายละเอียด">
                <Eye size={20} />
            </a>
            <button 
