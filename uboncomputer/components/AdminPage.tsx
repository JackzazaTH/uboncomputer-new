...
const ProductManagement: React.FC<{ 
  ...
  onDelete: (id: number) => void;
    ...
    return (
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold">จัดการสินค้า ({filteredProducts.length})</h2>
        ...
        <div className="mb-6 grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="relative sm:col-span-1">
             ...
             <select
                ...
                className="w-full border rounded-lg bg-white text-gray-900 focus:outline-none focus:ring-2 focus:ring-orange-500 py-2 px-3"
            >
            ...
            >
                ...
                <option value="inStock">มีสินค้าในสต็อก</option>
                <option value="lowStock">สต็อกเหลือน้อย (≤10)</option>
...
const UserManagement: React.FC<{
  ...
  onDelete: (userId: number) => void;
    ...
    const handleSaveFromModal = (userDataFromForm: Partial<User>) => {
        ...
        if (isNewUser) {
            userToSave = {
                ...
                password: userDataFromForm.password || '', // Let form validation handle this
                role: userDataFromForm.role || 'user',
            };
        } else {
            const existingUser = users.find(u => u.id === userDataFromForm.id);
            if (!existingUser) {
                addToast('ไม่พบผู้ใช้', 'error');
                return;
            }
            userToSave = {
                ...existingUser,
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold">จัดการผู้ใช้ ({filteredUsers.length})</h2>
                <button
                    onClick={handleAdd}
  ...
  return (
    <div className="flex min-h-screen bg-gray-100 font-sans">
      ...
      <button
        ...
        className="fixed bottom-8 right-8 bg-blue-600 text-white font-bold py-3 px-5 rounded-full shadow-lg hover:bg-blue-700 transition-all duration-300 flex items-center gap-2 z-50 animate-slide-in-up"
        title="กลับไปหน้าบ้าน"
      >
        <Home size={20} />
        <span className="hidden sm:inline">ดูหน้าบ้าน</span>
      </button>
